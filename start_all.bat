...
if %ERRORLEVEL% NEQ 0 (
  echo ERROR: openssl was not found on PATH. Generate DeepConsol\certs\server.crt manually.
  pause
  exit /b 1
)

if not exist "DeepConsol\certs" mkdir "DeepConsol\certs"
openssl req -x509 -newkey rsa:2048 -nodes -keyout "DeepConsol\certs\server.key" -out "DeepConsol\certs\server.crt" -days 825 -subj "/CN=netact.local" -addext "subjectAltName=DNS:localhost,IP:127.0.0.1" 2>nul
